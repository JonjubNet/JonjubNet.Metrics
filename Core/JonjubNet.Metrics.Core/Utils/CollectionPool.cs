...
namespace JonjubNet.Metrics.Core.Utils
{
    /// <summary>
    ...
    {
        private static readonly ConcurrentQueue<List<MetricEvent>> _metricEventListPool = new();
        ...
        /// <summary>
        /// Obtiene una List<MetricEvent> del pool o crea una nueva
        /// </summary>
        public static List<MetricEvent> RentMetricEventList()
        {
            if (_metricEventListPool.TryDequeue(out var list))
            {
                return list;
            }
        ...
        /// <summary>
        /// Devuelve una List<MetricEvent> al pool
        /// </summary>
        public static void ReturnMetricEventList(List<MetricEvent> list)
        {
            if (list == null) return;
            list.Clear();
            if (_metricEventListPool.Count < MaxPoolSize)
            {
                _metricEventListPool.Enqueue(list);
            }
        }
        ...
        {
            while (_metricEventListPool.TryDequeue(out _)) { }
